...
class KartuKeluargaSeeder extends Seeder
{
    ...
    {
        // Use Indonesian locale for realistic names
        $faker = \Faker\Factory::create('id_ID');

        KartuKeluarga::query()->delete();
        Penduduk::query()->delete();

        // Create 30 Kartu Keluarga with members for better visualization
        for ($i = 1; $i <= 30; $i++) {
            ...
            $kk = KartuKeluarga::create([
                'nomor_kk' => '330509' . $faker->unique()->numerify('##########'),
                'alamat' => "$streetName No. $streetNumber",
                'rt' => str_pad(rand(1, 5), 3, '0', STR_PAD_LEFT),
                'rw' => str_pad(rand(1, 3), 3, '0', STR_PAD_LEFT),
                'kode_pos' => '54364',
                'desa_kelurahan' => 'Somagede',
                'kecamatan' => 'Sempor',
            ...
            // Create Kepala Keluarga (Head of Family)
            $lastName = $faker->lastName; // Shared last name for family cohesion (optional but nice)
            $bapakName = $faker->firstNameMale . ' ' . $lastName;

            Penduduk::create([
                'nik' => '330509' . $faker->unique()->numerify('##########'),
                'nama_lengkap' => $bapakName,
                'jenis_kelamin' => 'L',
            ...
            // Create Istri (Wife)
            $ibuName = $faker->firstNameFemale . ' ' . $lastName;
            Penduduk::create([
                'nik' => '330509' . $faker->unique()->numerify('##########'),
                'nama_lengkap' => $ibuName,
                'jenis_kelamin' => 'P',
                'tempat_lahir' => 'Kebumen',
            ...
            for ($j = 1; $j <= $numChildren; $j++) {
                ...
                $childName = ($gender == 'L' ? $faker->firstNameMale : $faker->firstNameFemale) . ' ' . $lastName;
                
                Penduduk::create([
                    'nik' => '330509' . $faker->unique()->numerify('##########'),
                    'nama_lengkap' => $childName,
                    'jenis_kelamin' => $gender,
                    'tempat_lahir' => 'Kebumen',
                    'tanggal_lahir' => $faker->dateTimeBetween('-20 years', '-1 years')->format('Y-m-d'),
                    'alamat' => $kk->alamat,
                    'rt' => $kk->rt,
                    'rw' => $kk->rw,
